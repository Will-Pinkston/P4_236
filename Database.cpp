...
{
//    relation retVal (rLeft.getName() + "_" + rRight.getName() + "_Union", rLeft.getMap());
...
relation database::relJoin (relation &rLeft, relation &rRight/*,
                  std::vector<std::string> leftCriteria,
                  std::vector<std::string> rightCriteria*/)
{
    ...
    {
        rMap.erase(rMap.begin() + rightFound[i]);
    }
    
    std::vector<std::string> joinMap = rLeft.getMapOriginal();
    for (int i = 0; i < rMap.size(); i++)
    {
        joinMap.push_back(rMap[i]);
    }
...
std::string database::fillRules()
{
    ...
    while (changed[0])
    {
        ...
        {
            if (i == 0) changed[i] = evalRule(_rules[i]);
            else
            {
                changed.push_back(evalRule(_rules[i]));
                changed[0] = changed[0] || changed[i];
            }
        }
        numPasses++;
    ...
    std::stringstream ss;
    ss << "Schemes populated after " << numPasses << " passes through the Rules.";
    return ss.str();
}
...
//        std::stringstream addName; addName << relName << "_" << i;
    ...
    for (int i = (int)predicateRelations.size() - 1; i > 0; i--)
    {
        ...
        predicateRelations.erase(predicateRelations.begin() + i);
    }
